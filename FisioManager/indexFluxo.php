...
?>
	...
	<div class="row" style="margin-left:10px; margin-top: 15px; margin-right: 10px;">
		<div class="col-md-12">
			...
			<div class="card">
				<div class="card-body">
					<h5 class="card-title">Entrada Quiropraxia
						<a title='Cadastrar Cliente' href='entradaCliente.php' accesskey="C">
							...
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-hover" id="table-indexFluxo" style="width: 100%">
									...
									<tbody id="searchTable">
										...
										$buscaCliente = $pdo
											->prepare(
												'SELECT c.idCliente AS codigo, c.nome AS cliente, c.cpf AS cpf, pl.plano AS plano,
												pg.tipo AS forma, c.procedimento AS tipo, c.dataEntrada AS dt_entrada,
												c.horaEntrada AS hr_entrada, c.dataSaida AS dt_saida, c.horaSaida AS hr_saida,
												c.saldo AS saldo, c.status_pago AS pago, c.pc_valor AS preco, c.template AS template
										FROM
											cliente c
										    JOIN pagamento pg
										    ON c.Pagamento_idPagamento = pg.idPagamento
											JOIN planos AS pl
										    ON c.Planos_idPlano = pl.idPlano ORDER BY c.idCliente DESC'
											);
											...
											foreach ($row as $value) { ?>
												...
												<tr>
													...
													<td style="font-size: 16px; text-align: center;">
														...
														if (strcmp($value['plano'], 'Experimental') === 0) {
															$sinalExp = $pdo->prepare("UPDATE cliente SET status_pago  = 3 WHERE idCliente = ?");
		...
		function Capture(id) {
			...
			success: function (data) {
				...
				if (data != "" && data != null) {
					...
					$.ajax({
						url: 'insertBiometriaQuiropraxia.php',
						type: 'POST',
						data:{
							template: data, 
							id:id
					...
					$.ajax({
						...
						success: function (data) {
							...
							if (data != "") {
								...
								// Passa via ajax biometria pra ser inserida no banco de dados.
								$.ajax({
									url: 'updateClienteQuiropraxiaBiometria.php',
									type: 'POST',
									data:{
										template: data, 
										id:id
