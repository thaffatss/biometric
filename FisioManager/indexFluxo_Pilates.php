								...
								<table class="table table-striped table-bordered table-hover" id="table-indexFluxo" style="width: 100%">
									...
									<tbody id="searchTable">
										...
										$buscaClientePilates = $pdo
											->prepare(
												'SELECT
											cp.idCliente_pilates AS codigo, cp.nome AS cliente, cp.cpf AS cpf, pl.plano AS plano,
											pg.tipo AS forma, cp.dataInicio AS dt_inicio, cp.template AS template,
										...
										FROM
											...
											JOIN planos AS pl
										    ON cp.Planos_idPlano = pl.idPlano ORDER BY cp.idCliente_pilates DESC'
										);
										...
										?>
												<tr>
													...
													<td style="font-size: 16px; text-align: center;">
														...
														if (strcmp($value['plano'], 'Experimental') === 0) {
															$sinalExp = $pdo->prepare("UPDATE cliente_pilates SET status_pago  = 3 WHERE idCliente_pilates = ?");
	...
	function Capture(id) {
		...
		success: function (data) {
			...
			if (data != "" && data != null) {
				...
				$.ajax({
					url: 'insertBiometriaPilates.php',
					type: 'POST',
					data:{
						template: data, 
						id:id
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (data != "") {
							...
							// Passa via ajax biometria pra ser inserida no banco de dados.
							$.ajax({
								url: 'updateClientePilatesBiometria.php',
								type: 'POST',
								data:{
									template: data, 
									id:id
