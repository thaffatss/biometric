										...
										$buscaCliente = $pdo
											->prepare(
												'SELECT cr.idCliente_reabilitacao AS codigo, cr.nome AS cliente, cr.cpf AS cpf, pl.plano AS plano,
													...
													cr.saldo AS saldo, cr.status_pago AS pago, cr.pc_valor AS preco, cr.template AS template
										FROM
											...
											foreach ($row as $value) { ?>
												...
												<tr>
													...
													<td style="font-size: 16px; text-align: center;">
														...
														if (strcmp($value['plano'], 'Experimental') === 0) {
															$sinalExp = $pdo->prepare("UPDATE cliente_reabilitacao SET status_pago  = 3 WHERE idCliente_reabilitacao = ?");
	...
	*********************************************/
	...
	success: function (data) {
		...
		if (data != "" && data != null) {
			...
			$.ajax({
				url: 'insertBiometriaReabilitacao.php',
				type: 'POST',
				data:{
					template: data, 
					id:id
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data != "") {
						...
						// Passa via ajax biometria pra ser inserida no banco de dados.
						$.ajax({
							url: 'updateClienteReabilitacaoBiometria.php',
							type: 'POST',
							data:{
								template: data, 
								id:id
