								...
								<tbody>
									<tr>
										<td style="width: 100%; text-align:center;">
											<h2>RELATÓRIO P/ TOMADA DE DECISÕES - PILATES</h2><br>
										</td>
										<td style="width: 10%;">
											<a title ='Gerar PDF' 
						...
						<div class="table-responsive">
							<table class="table table-striped table-bordered table-hover" 
								...
								<thead>
									<tr>
										<th width="20%">
											<center><font size=2>QTD.CADASTRADOS</font>
										</th>
										<th width="20%">
										...
										<th width="20%">
											<center><font size=2>INADIMPLÊNTES</font>
										</th>
									</tr>
								</thead>
								<tbody id="searchTable">
									...
									<?php
									$buscaCliente1 = $pdo
									->prepare("SELECT COUNT(cp.idCliente_pilates) AS qtdAtm, cp.status_pago FROM cliente_pilates cp WHERE cp.dataInicio BETWEEN'".$date1."'AND'".$date2."'");
									...
									$buscaCliente3 = $pdo
									->prepare("SELECT SUM(cp.pc_valor) AS subTotal, COUNT(cp.idCliente_pilates) AS qtdPago, cp.status_pago FROM cliente_pilates cp WHERE cp.dataInicio BETWEEN'".$date1."'AND'".$date2."' AND cp.status_pago = 1");
									...
									$buscaCliente4 = $pdo
									->prepare("SELECT COUNT(cp.idCliente_pilates) AS qtdPago, cp.status_pago FROM cliente_pilates cp WHERE cp.dataFim BETWEEN'".$date1."'AND'".$date2."' AND cp.dataFim > ?");
									$buscaCliente3->bindValue(1, date('m/d/Y'));
									$buscaCliente3->execute();
									$row = $buscaCliente3->fetchALL(PDO::FETCH_ASSOC);
									if(count($row) > 0) {
										foreach ($row as $key => $value3) { ?>
